...
import type { Task, TaskList } from "./model/task";
import { buildTaskTree } from "./model/viewModel/taskTree";

function App() {
	const loadedTaskList: TaskList = JSON.parse(
	...
	const tasks =
		loadedTaskList.tasks.length > 0 ? loadedTaskList : defaultTaskList;
			...
			produce((draft) => {
				draft.tasks = draft.tasks.map((t) => {
					...
					if (
						t.id === task.id &&
						t.order <
							draft.tasks.filter((t) => t.parentId === task.parentId).length - 1
					) {
						return { ...t, order: t.order + 1 };
					}
					if (t.order === task.order + 1) {
